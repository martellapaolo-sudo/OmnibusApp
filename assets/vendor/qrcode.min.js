/**
 * Nayuki QR Code Generator + High-Res SVG & Canvas Bridge
 * MIT License
 */
"use strict";
var qrcodegen;
...
if (typeof global !== "undefined") global.qrcodegen = qrcodegen;

function qrToSvg(qr, border = 4) {
    const parts = [];
    ...
    return "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 " + totalSize + " " + totalSize + "\" shape-rendering=\"crispEdges\" style=\"max-width:200px;width:100%;height:auto;border-radius:12px;background:#fff;padding:8px;\"><path fill=\"#fff\" d=\"M0,0h" + totalSize + "v" + totalSize + "H0z\"/><path fill=\"#000\" d=\"" + parts.join(" ") + "\"/></svg>";
}
...
window.qrcode = function(typeNumber, errorCorrectionLevel) {
    ...
    return {
        ...
        getModuleCount: function() {
            const qr = qrcodegen.QrCode.encodeText(_data, qrcodegen.QrCode.Ecc.LOW);
            return qr.size;
        },
        createSvgTag: function(cellSize, margin) {
            const qr = qrcodegen.QrCode.encodeText(_data, qrcodegen.QrCode.Ecc.LOW);
            return qrToSvg(qr, margin !== undefined ? margin : 4);
        },
        createDataURL: function(scale, margin) {
            const qr = qrcodegen.QrCode.encodeText(_data, qrcodegen.QrCode.Ecc.LOW);
            if (typeof document !== "undefined" && document.createElement) {
                const s = scale || 4;
                const m = margin !== undefined ? margin : 4;
                const canvas = document.createElement("canvas");
                const total = (qr.size + m * 2) * s;
                canvas.width = total;
                canvas.height = total;
